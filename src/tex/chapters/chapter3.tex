...
committed to the Axelrod-Python library to implement the spatial topology are
...
a more generalized representation of the edges should be selected. Thus edges
...
written. The test are used to ensure compatibility and ensure that the expected
...
which is a hosted git repository. This introduces us to another important aspect
...
invented by Linus Torvalds and span to life in April 2005.
...
In Figure~\ref{fig:github} conversation containing comments and corrections
can be seen.
...
\centering
    ...
    \centering
        \includegraphics[width=\linewidth]{chapter-three/cycle_network.png}
...
\hfill
    \begin{subfigure}[h]{0.52\textwidth}\centering
    \centering
        \includegraphics[width=\linewidth]{chapter-three/complete-network.png}
...
by adding these additional rules to the tournaments was only increasing their usage.
For that reason all the tournaments and their results are run in Raven, which
is the super computer of Cardiff University. All the scripts and pbs file, for
...
In the next section, an preliminary analysis of the experiments aforementioned
has been held. Followed by, an intense analysis in section~\ref{sub:analyzing_the_effect_of_the_topologies}.
...
results. However, one could argue that by participating in a larger number of
...
\end{table}
% I run the tournaments that he participated again and had the same results
...
In the cyclic and periodic lattice topology the highest winning ration achieved
...
given random situation. Further investigation will be conducted.
...
the cyclic and lattice, [10, 20, 30, 40, 50, 60, 70, 80, 90]. For tournament
...
respectively. The winning rations were plotted against participation number
...
of rations between the participations group is significant(\(p\) values
...
Equivalent, in ~\ref{fig:winning-ratio-participations-fifty}, violin plots for
...
experiments. The specific groups that differed are not clear.
Post-hoc tests could be performed there will be no more investigation to this.
...
\centering
    ...
    \caption{Winning Ration vs Number of Participations Cyclic Tournament Size 5.}
    \end{subfigure}
\hfill
    ...
    \caption{Winning Ration vs Number of Participations Periodic Lattice Tournament Size 5.}
    \end{subfigure}
\hfill
    \begin{subfigure}[t]{0.75\textwidth}\centering
    \centering
        \includegraphics[width=\linewidth]{chapter-three/participation-ratio-violin-Round-Robin-5.pdf}
    \caption{Winning Ration vs Number of Participations Round Robin Tournament Size 5.}
    \end{subfigure}
\caption{Winning Ration vs Number of Participations Tournament Size 5.}
...
\centering
    ...
    \caption{Winning Ration vs Number of Participations Cyclic Tournament Size 50.}
    \end{subfigure}
\hfill
    \begin{subfigure}[t]{0.75\textwidth}\centering
    \centering
        \includegraphics[width=\linewidth]{chapter-three/participation-ratio-violin-Periodic-Lattice-50.pdf}
    \caption{Winning Ration vs Number of Participations Periodic Lattice Tournament Size 50.}
    \end{subfigure}
\hfill
    \begin{subfigure}[t]{0.75\textwidth}\centering
    \centering
        \includegraphics[width=\linewidth]{chapter-three/participation-ratio-violin-Round-Robin-50.pdf}
    \caption{Winning Ration vs Number of Participations Round Robin Tournament Size 50.}
    \end{subfigure}
\caption{Winning Ration vs Number of Participations Tournament Size 50.}
\label{fig:winning-ratio-participations-fifty}
\end{figure}

\newpage
In this subsection the winning ratio has been covered. The winning ratio is defined
as the number of ranking first in a tournament divided by the number you participated
in one. The strategies were ranked based on their winning ratio in each of the
...
The first model was build with the winning ratio as the depended variable,
shown here:
...
From the results it is shown that only degree seems to be a stable significant
predictor for all three experiments. With a \(p\) value less that 0.001. Average
of size 50, it has a negative correlation with the winning ratio. Thus, the
...
Overall, all R-square values  are really low. With the highest R- value of a
...
an 0.0001.
% Less than 0 means negative. % we did that in statistics , less than 0.0001.
...
For both models and all set of 6 experiments some predictors can e characterized
as significant. More than one factors have a p value less than a 0.05, the alpha
that has been set. Even so, the coefficients and the overall R square values are
...
Moreover, it lists topic for further research to be conduct.
...
\begin{itemize}
  \item For the size 5 experiments, the well performed strategies were Punisher,
        Raider and BackStabber
  \item For the size 50 experiments, the well performed strategies were Nice
  ...
  \item Raider seem to have successful performance in general
\end{itemize}
...
high ranked winning rations. Even so, valid arguments as to how to
...
Because this kind experiments have not be conducted before.
The Axelrod- Python library tournament could be used  a measure of comparison.
Still two of the best overall strategies of that tournament have only made a
brief appearance.

Further actions to the experiments and can be taken for this point on.
Producing a more complete data set for each experiment and making a more
deep analysis. One could argue that all above experiments
were conducted in simple topologies and small number of repetitions. Thus, the next
step is to use more complex networks for topologies and conduct a larger number
of tournaments.
