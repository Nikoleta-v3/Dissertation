...
\begin{itemize}
	\item A cyclic network: the neighbourhood size is 2. % include specific references % for networks or tournaments ?
...
strategies loosing to an opponent, but their opponent participated in more
tournaments. Thus, their average score, divided by a bigger number, was lower.
%This was weird, but Tricky Cooperator played in a few matched where he was out
% performed by phi. So didn't get a win. But phi participated in more tournaments
% and his normalized score was lower
\begin{figure}[H]
	\centering
...
the ranking, except Raider. The example of Tricky Cooperator and Hard Tit For 2 Tats,
give enough reasons for the wining ratio and normalized score, being non satisfactory
measures. In the following subsection, a linear regression model is build for
more conclusions.
...
has been produced, by adding to an already existed python library. The Axelrod-Python
library. Secondly, an experiment using the new capabilities of the library has been
...
Creating a conflict and indicating the both measures did poorly.
