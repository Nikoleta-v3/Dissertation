...
More specifically, for the non deterministic objective function
...
Thus, for a non deterministic function the corresponding lookup table generates
...
For comparison reason an extra algorithm has been performed. This time the
objective function has been measured as the minimum normalized score and was carried
out for only the non deterministic function.The results
that were generated from this run are given by the Table~\ref{results-min-geten}.
The overall results have been lower than before. The finest score has been achieved for a sample
size of 15 to 50 and returned an minimum normalized score of 0.478.
...
are discussed. In the previous sections that stricture of the algorithm has been
explained, and because a similar approach of the Chapter 4 has been tried to
be kept the networks used in the scoring process have been randomly picked between
small world, random and complete. In this section each of network participated in the algorithm
and the individual results are studied. This was implemented mainly for comparison reasons.
Tables 1 and 2 illustrate the results.
...
The achievement in this section is to see that a strategy has been trained
probably for at least one of the topologies. Thus gives evidence that
at least one of networks achieved that then so could the rest. it we had more time.
